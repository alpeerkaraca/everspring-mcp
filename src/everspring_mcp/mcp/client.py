"""EverSpring MCP - Client for interacting with the MCP Server.
...
class MCPClient:
    """Client for interacting with EverSpring MCP server.
    ...
    def format_results(
        ...
        if response.filters_applied:
            filter_strs = [
                f"{k}={v}" for k, v in response.filters_applied.items()
            ]
        ...
        for result in response.results:
            ...
            lines.append(
                f"    Score: {result.score:.4f} ({result.score_breakdown})"
            )
